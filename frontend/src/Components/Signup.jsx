      ...
      <div className="w-full max-w-md p-8 bg-[#16243a] rounded-2xl shadow-lg">
        ...
        <form onSubmit={handleSignup} className="space-y-6">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} required className="w-full px-4 py-2 rounded-lg bg-[#22334d] text-white focus:outline-none focus:ring-2 focus:ring-teal-400" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required className="w-full px-4 py-2 rounded-lg bg-[#22334d] text-white focus:outline-none focus:ring-2 focus:ring-teal-400" />
          </div>
          {error && <div className="text-red-400 text-sm text-center">{error}</div>}
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-3 rounded-lg bg-teal-400 text-[#101c2a] font-bold text-lg shadow hover:bg-teal-500 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? "Creating Account..." : "Sign Up"}
          </button>
          <button type="button" onClick={handleGoogleSignup} className="w-full py-3 rounded-lg bg-white text-[#101c2a] font-bold text-lg shadow hover:bg-teal-200 transition flex items-center justify-center gap-2">
